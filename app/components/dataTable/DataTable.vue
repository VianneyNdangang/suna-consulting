<template>
  <div class="p-8 rounded-xl border border-(--border) bg-sand-50 shadow-sm overflow-auto w-full">
    ...
    <div v-else><p class="mt-2 text-lg font-semibold text-gray-800">
      List of {{ totalRecords }} {{ title }}.
    </p>

    <table class="w-full">
      <thead class="h-10 text-rust-900">
        <tr>
          <th
      ...
      <tbody>
        ...
        >
          <td
            v-for="header in props.headers"
            :key="header.accessor"
            :class="`px-3 py-2 border-b text-${header.textAlign} text-rust-600 border-(--border)`"
          >
            <component
